...
export default function NurseAlertsPage() {
  ...
  const handleResolve = async (alertId: number) => {
    try {
        const res = await api.post(`alerts/nurse/${alertId}/resolve/`, {
            resolution_notes: "Nurse acknowledged and managed incident."
        });
        if (res.data.success) {
            removeAlert(alertId);
        }
    ...
    <div className="space-y-10 w-full p-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
            <div className="space-y-4">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-500 italic">
                        Nurse Intervention Console
                    </span>
                </div>
                <h1 className="text-7xl font-black text-zinc-100 uppercase tracking-tighter leading-none">
                    Incident <span className="text-emerald-500">Queue</span>
                </h1>
                <p className="text-zinc-500 text-sm font-medium italic max-w-xl leading-relaxed">
                    Real-time clinical triggers requiring triage. Grouped by patient to ensure focused care delivery.
                </p>
            </div>

            <div className="flex gap-6">
                <div className="p-5 px-8 bg-emerald-500/10 rounded-[2rem] border border-emerald-500/20 backdrop-blur-md flex flex-col items-center">
                    <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">High Acuity</span>
        ...
        <div className="relative group mb-12">
            ...
            <Input
                ...
                className="pl-14 bg-zinc-900/20 border-zinc-800 text-zinc-100 h-16 rounded-[1.5rem] focus:ring-1 focus:ring-emerald-500/50 transition-all uppercase font-medium text-xs tracking-tight"
            />
        </div>
        ...
        <div className="space-y-12">
            {patientGroups.length > 0 ? (
                        ...
                        >
                            <div className="flex flex-wrap items-center justify-between gap-8 mb-10 border-b border-zinc-800/50 pb-10">
                                <div className="flex items-center gap-8">
                                    <div className={cn(
                                        ...
                                        hasCritical ? "bg-rose-600 text-white shadow-2xl shadow-rose-600/30" : "bg-emerald-600 text-white"
                                    )}>
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-black text-emerald-400/60 uppercase tracking-tighter">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <Button 
                                        onClick={async () => {
                                            const tId = toast.loading("Stabilizing Cluster...");
                                            try {
                                                let successCount = 0;
                                                for (const a of group) {
                                                    const res = await api.post(`alerts/nurse/${a.id}/resolve/`, {
                                                        resolution_notes: "Nurse stabilized cluster."
                                                    });
                                                    if (res.data.success) {
                                                        removeAlert(a.id);
                                                        successCount++;
                                                ...
                                                toast.success(`Stabilized ${successCount} incidents`, { id: tId });
                                            } catch (err) {
                                                toast.error("Stabilization failed.", { id: tId });
                                            }
                                        }}
                                        className={cn(
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                {paginatedAlerts.map((alert) => (
                                    <div 
                                        key={alert.id}
                                        className="p-6 rounded-[2rem] bg-zinc-950/40 border border-zinc-800/50 flex flex-col justify-between h-full hover:border-emerald-500/30 transition-all"
                                    >
                                        <div className="space-y-4 mb-8">
                                            <div className="flex justify-between items-start">
                                                <div className={cn(
                                                    "w-12 h-12 rounded-2xl flex items-center justify-center shrink-0",
                                                    alert.severity === "CRITICAL" ? "bg-rose-500/10 text-rose-500" : "bg-emerald-500/10 text-emerald-400"
                                                )}>
                                                    <Activity className="w-6 h-6" />
                                                </div>
                                                <span className={cn(
                                                    "text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-widest",
                                                    alert.severity === "CRITICAL" ? "border-rose-500/30 text-rose-500" : "border-emerald-500/30 text-emerald-400"
                                                )}>
                                                    {alert.severity}
                                                </span>
                                            </div>
                                        ...
                                        <div className="pt-6 border-t border-zinc-900 flex flex-col gap-4">
                                            ...
                                            <Button 
                                                ...
                                                className="w-full h-12 rounded-2xl bg-zinc-900 hover:bg-emerald-600 text-zinc-400 hover:text-white text-[9px] font-black uppercase tracking-widest transition-all gap-2"
                                            >
                ...
                <div className="flex flex-col items-center justify-center p-40 bg-zinc-950/20 rounded-[5rem] border border-zinc-900 backdrop-blur-sm">
                    ...
                    <h3 className="text-4xl font-black text-zinc-700 uppercase tracking-tight leading-none mb-4">Queue Normal</h3>
                    <p className="text-zinc-800 text-center max-w-md font-bold uppercase text-[11px] tracking-[0.2em] leading-relaxed">
                        No active clinical incidents reported in your sector. Systems are in monitoring mode.
                    </p>
                </div>
            )}
        </div>
